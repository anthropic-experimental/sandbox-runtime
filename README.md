...
- **`gh` (macOS/Linux)** - Go binaries on macOS verify TLS against the system keychain, not the CA-bundle env vars, and the sandbox blocks the `trustd` service they would need (see `enableWeakerNetworkIsolation`), so under TLS termination `gh` cannot verify the proxy-minted certificate. SRT sidesteps the handshake entirely: it listens on a private unix socket that speaks plain HTTP into the same terminated pipeline (domain allowlist, `filterRequest`, credential injection, with a verified TLS upstream leg) and points the sandboxed `gh` at it through its `http_unix_socket` setting. That setting is config-file only, so the child gets `GH_CONFIG_DIR` set to a per-session directory holding a copy of the user's `config.yml` with `http_unix_socket` set and a symlink to the user's real `hosts.yml` — auth state (`gh auth login`, tokens) is shared with the host in both directions, while `gh config set` / `gh alias set` run inside the sandbox only affect the copy. Covers everything that goes through gh's own HTTP client (REST, GraphQL, release/asset transfers, extension installs); gh extensions are separate binaries and `git` is unaffected (it honors `GIT_SSL_CAINFO`). Redirect targets such as `objects.githubusercontent.com` must still be in `allowedDomains`. Denials on this path carry no per-command attribution in `<sandbox_violations>` (there is no proxy credential to identify the caller); they are reported at session level with method, URL, and reason.
...
- `enableWeakerNetworkIsolation` - Allow access to `com.apple.trustd.agent` in the macOS sandbox (boolean, default: false). This is needed for Go programs (`gcloud`, `terraform`, `kubectl`, etc.) to verify TLS certificates when using `httpProxyPort` with a MITM proxy and custom CA (`gh` is handled without it under `network.tlsTerminate` — see that section). **Security warning:** enabling this opens a potential data exfiltration vector through the trustd service.
