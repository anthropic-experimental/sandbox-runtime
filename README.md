...
**Container seccomp profiles**: In Docker and Kubernetes, `CAP_SYS_ADMIN` is not always enough for nested PID namespace isolation. The default container seccomp profile can still block the `mount` syscall pattern used to mount a fresh `/proc` inside the nested namespace, producing errors such as `Can't mount proc on /newroot/proc: Operation not permitted`. To use the full nested sandbox in containers, run with a permissive/custom seccomp profile (or `--privileged`). For standard Docker/Kubernetes deployments with the default seccomp profile, set `enableWeakerNestedSandbox: true`; this skips the nested `/proc` remount while retaining the filesystem and network isolation that bubblewrap can enforce in that environment.
