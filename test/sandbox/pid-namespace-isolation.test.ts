...
describe.if(isLinux)('apply-seccomp PID namespace isolation', () => {
  beforeAll(() => {
    ...
    // If they're null, the entire suite is meaningless — fail loudly here
    // rather than silently skipping every test below.
...
describe.if(isLinux)(
  ...
  () => {
    beforeAll(() => {
      ...
      expect(applySeccomp).toBeTruthy()
      expect(bpfFilter).toBeTruthy()
      // On CI bwrap and socat are apt-installed; if they're missing the
      // suite below would silently no-op every test.
