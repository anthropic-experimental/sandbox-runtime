...
describe.if(isSupportedPlatform)('wrapWithSandbox customConfig', () => {
  ...
  describe('without customConfig', () => {
    it.if(isSupportedPlatform)('uses main config values', async () => {
  ...
  describe('with customConfig filesystem overrides', () => {
    it.if(isSupportedPlatform)(
      'uses custom allowWrite when provided',
      async () => {
        const command = 'echo hello'
    ...
    it.if(isSupportedPlatform)(
      'uses custom denyRead when provided',
      async () => {
  ...
  describe('with customConfig network overrides', () => {
    it.if(isSupportedPlatform)(
      'blocks network when allowedDomains is empty',
      async () => {
    ...
    it.if(isSupportedPlatform)(
      'uses main config network when customConfig.network is undefined',
      async () => {
        const command = 'echo hello'
        const wrapped = await SandboxManager.wrapWithSandbox(
          command,
  ...
  describe('readonly mode simulation', () => {
    it.if(isSupportedPlatform)(
      'can create a fully restricted sandbox config',
      async () => {
        const command = 'ls -la'
  ...
  describe('partial config merging', () => {
    it.if(isSupportedPlatform)(
      'only overrides specified filesystem fields',
      async () => {
    ...
    it.if(isSupportedPlatform)(
      'only overrides specified network fields',
      async () => {
        const command = 'echo test'
...
describe('empty allowedDomains network blocking (CVE fix)', () => {
  ...
  describe.if(isSupportedPlatform)(
    'SandboxManager.wrapWithSandbox with empty allowedDomains',
    () => {
      beforeAll(async () => {
      ...
      it.if(isSupportedPlatform)(
        'non-empty allowedDomains still works correctly',
        async () => {
          const result = await SandboxManager.wrapWithSandbox(
            command,
            undefined,
      ...
      it.if(isSupportedPlatform)(
        'undefined network config in customConfig falls back to main config',
        async () => {
          const result = await SandboxManager.wrapWithSandbox(
            command,
            undefined,
...
describe('allowWrite glob suffix handling', () => {
  ...
  it.if(isSupportedPlatform)(
    'allowWrite with /** suffix includes path in sandbox command',
    async () => {
  ...
  it.if(isSupportedPlatform)(
    'denyWrite with /** suffix within allowed parent includes both paths',
    async () => {
